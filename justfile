...
release-check:
    ...
    just package-smoke
    @cargo metadata --no-deps --format-version 1 | jq -e '.packages[0].version == "0.10.0"' >/dev/null
    @test -n "$$(awk '/^## \[0\.10\.0\]/{found=1} END{print found}' CHANGELOG.md)"
