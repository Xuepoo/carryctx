...
impl<'a> PresetManager<'a> {
    ...
    pub fn read_lockfile(&self) -> Result<PresetLockfile, CarryCtxError> {
        ...
        let content = fs::read_to_string(&lock_path)
            .map_err(|e| CarryCtxError::database_error(format!("Failed to read presets.lock: {e}")))?;

        toml::from_str(&content)
            .map_err(|e| CarryCtxError::database_error(format!("Failed to parse presets.lock: {e}")))
    }
    ...
    pub fn write_lockfile(&self, lockfile: &PresetLockfile) -> Result<(), CarryCtxError> {
        ...
        if let Some(parent) = lock_path.parent() {
            fs::create_dir_all(parent)
                .map_err(|e| CarryCtxError::database_error(format!("Failed to create .carryctx dir: {e}")))?;
        }

        let content = toml::to_string_pretty(lockfile)
            .map_err(|e| CarryCtxError::database_error(format!("Failed to serialize presets.lock: {e}")))?;
        ...
        fs::write(&lock_path, final_content)
            .map_err(|e| CarryCtxError::database_error(format!("Failed to write presets.lock: {e}")))
    }
    ...
    pub fn install_preset(&self, source_path: &Path) -> Result<PresetLockEntry, CarryCtxError> {
        let content = fs::read_to_string(source_path).map_err(|e| {
            CarryCtxError::resource_not_found(format!("Failed to read preset source {}: {}", source_path.display(), e))
        })?;
        ...
        fs::create_dir_all(&presets_dir)
            .map_err(|e| CarryCtxError::database_error(format!("Failed to create presets directory: {e}")))?;
        ...
        fs::write(&target_file, &content).map_err(|e| {
            CarryCtxError::database_error(format!("Failed to save preset to {}: {}", target_file.display(), e))
        })?;
        ...
        lockfile.presets.insert(manifest.name.clone(), entry.clone());
    ...
    pub fn activate_preset(&self, name: &str) -> Result<PresetLockEntry, CarryCtxError> {
        ...
        let entry = lockfile.presets.get(name).cloned().ok_or_else(|| {
            CarryCtxError::resource_not_found(format!("Preset '{}' is not installed in presets.lock", name))
        })?;
        ...
        let content = fs::read_to_string(&preset_path).map_err(|e| {
            CarryCtxError::database_error(format!("Failed to read {}: {}", preset_path.display(), e))
        })?;
